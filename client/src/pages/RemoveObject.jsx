import { CodeXmlIcon, Scissors } from 'lucide-react'
...
setLoading(false)
   ...
   <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
         <form  onSubmit={onSubmitHandler}  className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'  >
           <div className='flex items-center gap-3' >
             <CodeXmlIcon className='w-6 text-red-700' />
 ...
 <p className='mt-6 text-sm font-medium'> Describe object name to remove</p>
        ...
        <textarea onChange={(e)=> setObject(e.target.value)} value={object} rows={4}  className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder=' for example., watch or spoon, only single object name....' required />
  ...
  <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96'>
   <div className='flex items-center gap-3' > 
   <Scissors className='w-5 h-5 text-[#e6080f]'  />
...

{
  !content ? (
   ...
   <p>Upload an image and click "Remove Object" with WorkLr.ai</p>
     </div>
   </div>
  ) : (
