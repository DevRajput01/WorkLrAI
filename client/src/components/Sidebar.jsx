...
import { BookAIcon, Eraser, File, HomeIcon, ImageIcon, LogOut, PenSquare, ScissorsIcon, User} from 'lucide-react'
import { NavLink } from 'react-router-dom'

const navItems = [
    {to: '/ai', label: 'Dashboard', Icon: HomeIcon},
    {to: '/ai/write-article', label: 'Write Article', Icon: PenSquare},
    {to: '/ai/blog-titles', label: 'Blog Titles', Icon: BookAIcon},
    {to: '/ai/generate-images', label: 'Generate Images', Icon: ImageIcon},
    {to: '/ai/removebg', label: 'Remove Bg', Icon: Eraser},
...
<img src={user.imageUrl} alt="UserAvatar" className='w-13 rounded-full mx-auto cursor-pointer'/>
<h1 className='mt-1 text-center' >{user.fullName}</h1>
<div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
    {navItems.map(({to, label, Icon})=>(
        <NavLink key={to} to={to} end={to === '/ai'} onClick={()=>
        setSidebar(false)} className={({isActive})=> `px-3.5 py-2.5 flex
         items-center gap-3 rounded ${isActive ? 'bg-gradient-to-r from-[#4103bb] to-[#8c52ff] text-white': ''}`} >
            {( {isActive})=>(
                <>
